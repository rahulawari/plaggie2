 ...
 */
public class SimpleTokenSimilarityChecker implements TokenSimilarityChecker {
	...
	private CodeExcluder codeExcluder = new CodeExcluder() {
		public void mark(TokenList tokensA, TokenList tokensB,
				boolean[] marksA, boolean[] marksB) {
		}
	};

	public SimpleTokenSimilarityChecker(int minimumMatchLength) {
		this(minimumMatchLength, new CodeExcluder() {
			public void mark(TokenList tokensA, TokenList tokensB,
					boolean[] marksA, boolean[] marksB) {
			}
		});
	...
	public SimpleTokenSimilarityChecker(int minimumMatchLength,
		...
		if (codeExcluder == null) {
			throw new IllegalArgumentException("Illegal code excluder: "
					+ codeExcluder);
		}
	...
	/**
	 * Algorithm from [5], simple, straigthforward and unoptimized
	 * implementation
	 */
	public void countSimilarities(TokenList tokensA, TokenList tokensB) {
		...
		boolean[] marksA = new boolean[tokensA.size()];
		boolean[] marksB = new boolean[tokensB.size()];
		...
		this.codeExcluder.mark(tokensA, tokensB, marksA, marksB);

		// The current tile id, will be increased when tiles are added to the
		// final set
		int curId = 0;
		do {
			...
			for (int a = 0; a < tokensA.size(); a++) {
				if (!marksA[a]) {
					for (int b = 0; b < tokensB.size(); b++) {
						if (!marksB[b]) {
							int j = 0;
							while (((a + j) < tokensA.size())
									&& ((b + j) < tokensB.size())
									&& (!marksA[a + j])
									&& (!marksB[b + j])
							...
							if (j == maxMatch) {
								Debug.println(this,
										"Found maxLength tile of length " + j
												+ " at A:" + a + " B:" + b);
								Tile tileA = new Tile(tokensA, a, a + j - 1);
								Tile tileB = new Tile(tokensB, b, b + j - 1);
								MatchedTile mt = new MatchedTile(tileA, tileB);
								boolean check = matches
										.addMatchedTileNoOverlap(mt);
								Debug.println(this,
										"Was tile added to matches: " + check);
							} else if (j > maxMatch) {
								Debug.println(this, "Found tile of length " + j
										+ " at A:" + a + " B:" + b);
								matches = new MatchedTileSet();
								Tile tileA = new Tile(tokensA, a, a + j - 1);
								Tile tileB = new Tile(tokensB, b, b + j - 1);
			...
			while (matchedTileIter.hasNext()) {
				...
				Debug.println(this, "Marking tiles of length " + maxMatch
						+ " A:" + mt.getTileA().getStartTokenIndex() + ", B:"
						+ mt.getTileB().getStartTokenIndex() + ".");
	...
	public double getSimilarityValueA() throws IllegalStateException {
		if (tokensA == null) {
	...
	public double getSimilarityValueB() throws IllegalStateException {
		if (tokensA == null) {
			throw new IllegalStateException("Similarity count not yet run.");
		}
	...
	public MatchedTileSet getSimilarityTiles() throws IllegalStateException {
		if (tokensA == null) {
			throw new IllegalStateException("Similarity count not yet run.");
		}
		return tiles;
